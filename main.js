...
var secscan = function(opts,cb) {
    if (false === (this instanceof secscan)) {
        return new secscan(opts,cb);
    }
...
util.inherits(secscan, events);

secscan.prototype.getOptions = function() {
    return this.options;
};

secscan.prototype.initTestLocalhost = function() {
    if (
...
secscan.prototype.fireProgress = function() {
    var o = this.q.stats();
    if (!this.paused) {
...
secscan.prototype.initQueueProgress = function() {
...
secscan.prototype.initQueue = function() {
...
secscan.prototype.pause = function() {
    if (this.paused) return;
    this.paused = true;
    this.q.pause(true);
};

secscan.prototype.unpause = function() {
    if (!this.paused) return;
    this.paused = false;
    this.q.pause(false);
};

secscan.prototype.initQueuePause = function() {
...
secscan.prototype.init = function() {
...
secscan.prototype.lookupGeo = function(ip,cb) {
...
secscan.prototype.lookupDns = function(ip,cb) {
    var self = this;
...
secscan.prototype.portScan = function(ip,port,cb) {
    if (!port) return cb();
    if (port == 0) return cb();
...
secscan.prototype.resultAddGeo = function(result,r) {
...
secscan.prototype.resultAddDns = function(result,r) {
...
secscan.prototype.resultAddPort = function(result,r) {
...
secscan.prototype.resultClean = function(r) {
    if (!r) {
        return r;
    }
...
secscan.prototype.scan = function(args,nextIteration) {
...
secscan.prototype.run = function(cb) {
    this.cbrun = cb;
    this.q.run();
    return;
};

secscan.prototype.abort = function() {
    this.q.abort();
    return;
};

secscan.prototype.getInfo = function() {
    return this.info;
};

module.exports = secscan;
